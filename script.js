...
async function upsertWeaponStockEntry(name, moneyType, purchasePrice, salePrice, quantity) {
...
async function saveWeapon() {
    ...
    const propre = {
        ...
        sale: parseFloat(document.getElementById('weapon-sale-propre').value.replace(/\s/g, '') || '0')
    };
    const sale = {
        ...
        sale: parseFloat(document.getElementById('weapon-sale-sale').value.replace(/\s/g, '') || '0')
    };
    ...
    if (editingWeaponId) {
        ...
        await updateWeaponGroupStock(name, propre, sale);
    } else {
        if (propre.qty <= 0 && sale.qty <= 0) {
            alert('Indiquez une quantité propre et/ou sale');
            return;
        }
        if (propre.qty > 0 && (!propre.sale || isNaN(propre.sale))) {
            alert('Prix vente propre requis');
            return;
        }
        if (sale.qty > 0 && (!sale.sale || isNaN(sale.sale))) {
            alert('Prix vente sale requis');
            return;
        }
        if (propre.qty > 0) await upsertWeaponStockEntry(name, 'propre', propre.purchase, propre.sale, propre.qty);
        if (sale.qty > 0) await upsertWeaponStockEntry(name, 'sale', sale.purchase, sale.sale, sale.qty);
...
async function updateWeaponGroupStock(name, propre, sale) {
    ...
    if (propre.qty > 0) {
        if (wPropre) {
            await saveWeaponToDB({
                ...
                salePrice: propre.sale,
                quantity: propre.qty,
                moneyType: 'propre'
            });
        } else {
            await upsertWeaponStockEntry(name, 'propre', propre.purchase, propre.sale, propre.qty);
        }
    } else if (wPropre) {
        await saveWeaponToDB({
            id: wPropre.id,
    ...
    if (sale.qty > 0) {
        if (wSale) {
            ...
            await upsertWeaponStockEntry(name, 'sale', sale.purchase, sale.sale, sale.qty);
        }
    } else if (wSale) {
        await saveWeaponToDB({
            id: wSale.id,
...
function editWeaponGroup(name) {
    ...
    document.getElementById('weapon-sale-propre').value = formatNumberWithSpaces(
        Math.round(wPropre?.salePrice ?? ref.propre ?? 0)
    );
    ...
    document.getElementById('weapon-sale-sale').value = formatNumberWithSpaces(
        Math.round(wSale?.salePrice ?? ref.sale ?? 0)
    );
