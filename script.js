...
function createAdminInventoryCard(weapon) {
    ...
    const weaponNameEscaped = weapon.name.replace(/'/g, "\\'");
    const canSellPropre = (weapon.qtyPropre || 0) > 0;
    const canSellSale = (weapon.qtySale || 0) > 0;
    ...
    card.innerHTML = `
        ...
        <div class="admin-sell-buttons">
            <button class="btn-sell" onclick="openSellModal('${weaponNameEscaped}', 'propre')" ${!canSellPropre ? 'disabled' : ''}>Vendu propre</button>
            <button class="btn-sell btn-sell-dirty" onclick="openSellModal('${weaponNameEscaped}', 'sale')" ${!canSellSale ? 'disabled' : ''}>Vendu sale</button>
        </div>
    `;
