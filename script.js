...
async function initializeWithFirebase() {
  ...
  allow read, write: if true;
    ...
    if (typeof loadPriceCatalogFromFirestore === 'function') {
        ...
        if (typeof refreshWeaponCatalogDatalists === 'function') refreshWeaponCatalogDatalists();
    }
...
function setupEventListeners() {
    ...
    if (weaponNameInput) {
        let weaponNameFillTimer = null;
        const scheduleWeaponCatalogFill = () => {
            clearTimeout(weaponNameFillTimer);
            weaponNameFillTimer = setTimeout(() => applyWeaponCatalogPricesToForm(), 200);
        };
        weaponNameInput.addEventListener('input', scheduleWeaponCatalogFill);
        weaponNameInput.addEventListener('change', () => applyWeaponCatalogPricesToForm());
        weaponNameInput.addEventListener('blur', () => applyWeaponCatalogPricesToForm());
    }

    const catalogWeaponNameInput = document.getElementById('catalog-weapon-name');
    if (catalogWeaponNameInput) {
        let catalogFillTimer = null;
        catalogWeaponNameInput.addEventListener('input', () => {
            clearTimeout(catalogFillTimer);
            catalogFillTimer = setTimeout(() => applyWeaponCatalogPricesToForm('catalog'), 200);
        });
        catalogWeaponNameInput.addEventListener('change', () =>
            applyWeaponCatalogPricesToForm('catalog')
        );
    }
...
function setPriceInputIfEmpty(inputId, value) {
    const input = document.getElementById(inputId);
    if (!input || value == null || isNaN(value) || value <= 0) return;
    if (input.value.replace(/\s/g, '')) return;
    input.value = formatNumberWithSpaces(Math.round(value));
}

/** Remplit les prix depuis le catalogue (gestion ou référence prix) */
async function applyWeaponCatalogPricesToForm(mode) {
    const isCatalog = mode === 'catalog';
    const nameInput = document.getElementById(isCatalog ? 'catalog-weapon-name' : 'weapon-name');
    if (!nameInput) return;
    ...
    if (isCatalog) {
        setPriceInputIfEmpty('catalog-sale-propre', cat.salePricePropre);
        setPriceInputIfEmpty('catalog-sale-sale', cat.salePriceSale);
        setPriceInputIfEmpty('catalog-purchase-propre', cat.purchasePricePropre);
        setPriceInputIfEmpty('catalog-purchase-sale', cat.purchasePriceSale);
        return;
    }

    const ref =
        typeof getReferencePricesForName === 'function' ? getReferencePricesForName(name) : {};
    setPriceInputIfEmpty('weapon-sale-propre', ref.propre ?? cat.salePricePropre);
    setPriceInputIfEmpty('weapon-sale-sale', ref.sale ?? cat.salePriceSale);
    setPriceInputIfEmpty('weapon-purchase-propre', cat.purchasePricePropre);
    setPriceInputIfEmpty('weapon-purchase-sale', cat.purchasePriceSale);
}

window.applyWeaponCatalogPricesToForm = applyWeaponCatalogPricesToForm;
...
function setupFirestoreListeners() {
    ...
    firestoreListeners.weapons = window.firebaseOnSnapshot(
        ...
        async (snapshot) => {
            ...
            await loadAdminWeaponsList();
            await loadAdminInventory();
            if (typeof refreshWeaponCatalogDatalists === 'function') {
                refreshWeaponCatalogDatalists();
            }
            if (typeof syncAnnonceWithStock === 'function') {
                const annonceTabActive = document
                    .getElementById('admin-tab-annonce')
                    ?.classList.contains('active');
                await syncAnnonceWithStock({ persist: true, render: annonceTabActive });
            }
        },
        (error) => {
