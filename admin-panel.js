...
function normalizeAnnonceWeaponName(name) {
    return (name || '').trim().toLowerCase();
}

function getStockWeaponNamesForAnnonce() {
    const weapons = typeof getWeapons === 'function' ? getWeapons() : [];
    const grouped =
        typeof groupWeaponsByName === 'function' ? groupWeaponsByName(weapons, true) : {};
    return Object.keys(grouped).sort((a, b) => b.length - a.length);
}

function getAllKnownWeaponNamesForMatching() {
    const names = new Set(getStockWeaponNamesForAnnonce());
    if (typeof getWeapons === 'function') {
        getWeapons().forEach((w) => {
            if (w.name) names.add(w.name);
        });
    }
    if (typeof getPriceCatalogMap === 'function') {
        Object.values(getPriceCatalogMap()).forEach((e) => {
...
function extractWeaponNameFromAnnonceLine(line, candidateNames) {
    const trimmed = (line || '').trim();
    if (!trimmed) return null;
    const lower = trimmed.toLowerCase();
    const names = candidateNames || getAllKnownWeaponNamesForMatching();
...
function buildFreshAnnonceLineMap(freshLines) {
    const lines = freshLines || buildAnnonceLinesFromStock();
    const stockNames = getStockWeaponNamesForAnnonce();
    const map = {};
    lines.forEach((line) => {
        const name = extractWeaponNameFromAnnonceLine(line, stockNames);
        if (name) map[normalizeAnnonceWeaponName(name)] = line;
    });
    return map;
}

function mergeAnnonceLinesWithSaved(savedLines, freshLines) {
    const fresh = freshLines || buildAnnonceLinesFromStock();
    const freshMap = buildFreshAnnonceLineMap(fresh);
    const stockKeys = new Set(
        getStockWeaponNamesForAnnonce().map((n) => normalizeAnnonceWeaponName(n))
    );
    const result = [];
    const seen = new Set();

    savedLines.forEach((line) => {
        const name = extractWeaponNameFromAnnonceLine(line);
        if (!name) return;
        const key = normalizeAnnonceWeaponName(name);
        if (!stockKeys.has(key) || !freshMap[key]) return;
        if (!seen.has(key)) {
            result.push(freshMap[key]);
            seen.add(key);
        }
    });
...
async function syncAnnonceWithStock(options = {}) {
    const { persist = true, render = true } = options;
    const fresh = buildAnnonceLinesFromStock();
    const saved = await loadSavedAnnonceLines();
    const before =
        saved && saved.length > 0 ? [...saved] : annonceLinesState.length ? [...annonceLinesState] : [];
    annonceLinesState =
        saved && saved.length > 0 ? mergeAnnonceLinesWithSaved(saved, fresh) : fresh;

    const changed =
        before.length !== annonceLinesState.length ||
        before.some((line, i) => line !== annonceLinesState[i]);

    if (persist && changed && (saved?.length > 0 || annonceLinesState.length > 0)) {
        await persistAnnonceLines([...annonceLinesState]);
    }
...
async function refreshAnnoncePreview() {
    await syncAnnonceWithStock({ persist: true, render: true });
}
...
window.syncAnnonceWithStock = syncAnnonceWithStock;
