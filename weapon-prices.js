...
function resolveDualPrices(weaponGroup) {
    ...
    // Les prix de référence ne modifient pas l'affichage stock/annonce (uniquement suggestions à l'ajout)
...
async function applyCatalogPricesToStockEntries(weapons) {
    let updated = 0;
    for (const entry of priceCatalogCache) {
        ...
        for (const w of matching) {
            ...
            const needsUpdate =
                Math.abs((w.purchasePrice || 0) - purchasePrice) > 0.01 ||
                Math.abs((w.salePrice || 0) - salePrice) > 0.01 ||
                Math.abs((w.salePricePropre || 0) - entry.salePricePropre) > 0.01 ||
                Math.abs((w.salePriceSale || 0) - entry.salePriceSale) > 0.01;
...
async function syncStockFromCatalog() {
    ...
    const updated = await applyCatalogPricesToStockEntries(weapons);
    ...
    } else if (typeof refreshAnnoncePreview === 'function') {
        await refreshAnnoncePreview();
    }
    showNotification(`${updated} entrée(s) de stock mises à jour`, 'success');
}

function getCatalogWeaponNames() {
    return priceCatalogCache.map((e) => e.name).filter(Boolean);
}

function refreshWeaponCatalogDatalists() {
    const names = new Set(getCatalogWeaponNames());
    ...
    ['weapon-catalog-datalist', 'catalog-weapon-datalist'].forEach((id) => {
...
function initWeaponPricesModule() {
    ...
    loadPriceCatalogFromFirestore().then(() => refreshWeaponCatalogDatalists());
}
...
window.refreshWeaponCatalogDatalists = refreshWeaponCatalogDatalists;
window.getCatalogWeaponNames = getCatalogWeaponNames;
